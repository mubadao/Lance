...
//	NSLog(@"messageData:%@", [message data]);
	[message release];
...
//    NSLog(@"All Header:%@", [(NSHTTPURLResponse *)response allHeaderFields]);
    if ([(NSHTTPURLResponse *)response statusCode] != 200)
    {
		m_error = [[NSError errorWithDomain:kAMFRemotingCallErrorDomain code:kAMFServerErrorErrorCode userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"The server returned status code %d at URL %@.", [(NSHTTPURLResponse *)response statusCode], [[response URL] absoluteString]], NSLocalizedDescriptionKey, [NSNumber numberWithInt:[(NSHTTPURLResponse *)response statusCode]], kAMFServerStatusCodeKey, nil]] retain];
	}
	else if ([[[(NSHTTPURLResponse *)response allHeaderFields] objectForKey:@"Content-Type"]
              rangeOfString:@"application/x-amf"].location == NSNotFound)
    {
		m_error = [[NSError errorWithDomain:kAMFRemotingCallErrorDomain code:kAMFInvalidResponseErrorCode userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"The server returned no application/x-amf data at URL %@.", [[response URL] absoluteString]] forKey:NSLocalizedDescriptionKey]] retain];
	}
