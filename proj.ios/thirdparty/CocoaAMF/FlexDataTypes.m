...
- (NSString *)description{
	return [NSString stringWithFormat:@"<%@ = 0x%08x> %@", [self className], (unsigned int)self, source];
}
...
timeToLive: %f, timestamp: %f,\nheaders:\n%@,\nbody:\n%@", [self className], (unsigned int)self, clientId,
...
- (NSString *)description{
	return [NSString stringWithFormat:@"<%@ = 0x%%08x faultCode: %@, faultDetail: %@, faultString: %@", 
		[self className], (unsigned int)self, faultCode, faultDetail, faultString];
}
