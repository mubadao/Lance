...
- (id)initWithData:(NSData *)data{
	if (self = [super init]){
//		NSLog(@"AMFActionMessage:data_size[%d]", data.length);
...
- (NSString *)description{
	return [NSString stringWithFormat:@"<%@ = 0x%08X | version: %d | headers: %d bodies: %d>\nheaders:\n%@\nbodies:\n%@", 
		[self class], (unsigned int)self, m_version, [m_headers count], [m_bodies count],
		m_headers, m_bodies];
}
...
- (NSString *)description{
	return [NSString stringWithFormat:@"<%@ = 0x%08X | targetURI: %@ | responseURI: %@>\n%@", 
		[self class], (unsigned int)self, m_targetURI, m_responseURI, m_data];
}
